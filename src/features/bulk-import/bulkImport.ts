...
const questInput = z.object({ title: z.string().min(1).max(160), notes: z.string().max(2000).default(''), priority: z.enum(['low', 'medium', 'high']).default('medium'), status: z.enum(['now', 'next', 'later', 'done']).default('next'), xp: z.number().int().min(0).max(100).default(15), dueDate: z.string().max(10).optional(), skillName: z.string().max(100).optional() }).strict()
...
export type BulkSummary = { skills: number; quests: number; knowledgeNotes: number; resources: number; incomeSources: number; cvs: number; advice: number }
export type BulkParseResult = { status: 'valid'; data: BulkImport; summary: BulkSummary } | { status: 'invalid'; reason: string }

const summarize = (data: BulkImport): BulkSummary => ({ skills: data.skills.length, quests: data.quests.length, knowledgeNotes: data.knowledgeNotes.length, resources: data.resources.length, incomeSources: data.incomeSources.length, cvs: data.cvs.length, advice: data.advice.length })
...
export function applyBulkImport(state: AppState, raw: string, makeId: () => string = () => crypto.randomUUID()): AppState {
  ...
  const quests: Quest[] = [...data.quests.map((input) => ({ id: makeId(), profileId, title: input.title, notes: input.notes, priority: input.priority, status: input.status, xp: input.xp, dueDate: input.dueDate, skillId: skillId(input.skillName) })), ...state.quests]
  ...
  return { ...state, profiles, skills, quests, knowledgeNotes, resources, incomeSources, cvs, advice, moneyPlan: data.moneyPlan ? { ...state.moneyPlan, ...data.moneyPlan } : state.moneyPlan }
}
...
export const bulkImportTemplate = JSON.stringify({
  ...
  quests: [{ title: 'Concrete next action', notes: '', priority: 'medium', status: 'next', xp: 15, skillName: 'Example skill' }],
