...
export function BulkImportPanel() {
  ...
  return <section className="panel bulk-import-panel">
    ...
    <p>Paste one validated JSON object to add or update many skills, quests, knowledge notes, resources, income sources, CVs, advice items, profile fields, and money-plan fields at once. Skill names are upserted instead of duplicated.</p>
    ...
    {summary && <div className="import-summary" aria-label="Import preview">{Object.entries(summary).map(([label, count]) => <span key={label}><b>{count}</b>{label}</span>)}</div>}
