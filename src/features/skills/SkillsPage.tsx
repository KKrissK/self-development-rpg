...
import { AlertTriangle, Check, FileCheck2, Gauge, LayoutGrid, List, Plus, Sparkles, Trash2, X } from 'lucide-react'
...
function SkillEditor({ skill, onSave, onCancel }: { skill?: Skill; onSave: (draft: SkillDraft) => void; onCancel: () => void }) {
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    onSave({
      name: String(form.get('name')).trim(), category: String(form.get('category')).trim(), level: Number(form.get('level')), status: form.get('status') as Skill['status'], evidence: String(form.get('evidence')).trim(),
...
function SkillDialog({ skill, onClose, onEdit }: { skill: Skill; onClose: () => void; onEdit: () => void }) {
  ...
  return <div className="skill-dialog-backdrop" role="presentation" onMouseDown={onClose}><section className="skill-dialog" role="dialog" aria-modal="true" aria-labelledby="skill-dialog-title" onMouseDown={(event) => event.stopPropagation()}>
    <header><div><span className="kind">{skill.category || 'General'}</span><h2 id="skill-dialog-title">{skill.name}</h2><p>{skill.status} · current level {skill.level}</p></div><button className="icon-btn" autoFocus aria-label="Close skill details" onClick={onClose}><X size={19}/></button></header>
    <div className="skill-dialog-level"><b>{skill.level}<small>/10</small></b><div><span><i style={{ width: `${skill.level * 10}%` }}/></span><small>Current level {skill.level}</small></div></div>
...
export function SkillsPage() {
  ...
  const [addMethod, setAddMethod] = useState<'manual' | 'ai-create' | 'ai-assess' | 'ai-estimate'>('ai-assess')
  ...
  function remove(skill: Skill) {
    ...
    update((current) => ({ ...current, skills: current.skills.filter((item) => item.id !== skill.id), quests: current.quests.map((goal) => { const skillIds = (goal.skillIds ?? (goal.skillId ? [goal.skillId] : [])).filter((id) => id !== skill.id); return { ...goal, skillId: skillIds[0], skillIds } }), achievements: current.achievements.map((achievement) => { const skillIds = (achievement.skillIds ?? (achievement.skillId ? [achievement.skillId] : [])).filter((id) => id !== skill.id); return { ...achievement, skillId: skillIds[0], skillIds } }), resources: current.resources.map((resource) => resource.skillId === skill.id ? { ...resource, skillId: undefined } : resource) }))
  ...
  return <div className="page skills-page">
    ...
    {adding && <section className="add-workbench"><div className="add-method-switch four" role="tablist" aria-label="Skill add method"><button role="tab" aria-selected={addMethod === 'ai-assess'} className={addMethod === 'ai-assess' ? 'active' : ''} onClick={() => setAddMethod('ai-assess')}><Check size={17}/><span><b>Detailed AI assessment</b><small>More accurate interview with a knowledge check</small></span></button><button role="tab" aria-selected={addMethod === 'ai-estimate'} className={addMethod === 'ai-estimate' ? 'active' : ''} onClick={() => setAddMethod('ai-estimate')}><Gauge size={17}/><span><b>Quick AI estimate</b><small>Simple experience questions, no test</small></span></button><button role="tab" aria-selected={addMethod === 'ai-create'} className={addMethod === 'ai-create' ? 'active' : ''} onClick={() => setAddMethod('ai-create')}><Sparkles size={17}/><span><b>AI generate</b><small>Create several skills from one description</small></span></button><button role="tab" aria-selected={addMethod === 'manual'} className={addMethod === 'manual' ? 'active' : ''} onClick={() => setAddMethod('manual')}><Plus size={17}/><span><b>Manual</b><small>Add a skill and optional profile yourself</small></span></button></div>{addMethod === 'manual' ? <SkillEditor onSave={add} onCancel={() => setAdding(false)}/> : <AiImportPanel key={addMethod} target="skills" mode={addMethod === 'ai-assess' ? 'assess' : addMethod === 'ai-estimate' ? 'estimate' : 'create'}/>}</section>}
