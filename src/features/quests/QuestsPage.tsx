...
export function GoalsPage({ onOpenTasks }: { onOpenTasks: (goalId: string) => void }) {
  ...
  return <div className={`page quests-page goals-page ${view === 'history' ? 'history-selected' : ''}`}>
    ...
    <div className="tabs goal-page-tabs" role="tablist" aria-label="Goals page sections"><button role="tab" aria-selected={view === 'active'} className={view === 'active' ? 'active' : ''} onClick={() => setView('active')}>Active Goals <span>{activeGoals.length}</span></button><button role="tab" aria-selected={view === 'history'} className={view === 'history' ? 'active' : ''} onClick={() => { setCreateOpen(false); setView('history') }}>Completion history <span>{doneGoals.length}</span></button></div>
