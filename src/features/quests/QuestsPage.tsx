...
function GoalCard({ goal, skillName, resources, tasks, locale, onStatus, onComplete, onEdit, onDelete, onOpenTasks }: { goal: Quest; skillName?: string; resources: LearningResource[]; tasks: GoalTask[]; locale: string; onStatus: (status: QuestStatus) => void; onComplete: () => void; onEdit: () => void; onDelete: () => void; onOpenTasks: () => void }) {
...
export function GoalsPage({ onOpenTasks }: { onOpenTasks: (goalId: string) => void }) {
  ...
  const effectiveAiSkillId = profileSkills.some((skill) => skill.id === aiSkillId) ? aiSkillId : profileSkills[0]?.id ?? ''
  ...
  return <div className="page quests-page goals-page">
    ...
    {createOpen && <section className="add-workbench"><div className="add-method-switch" role="tablist" aria-label="Goal creation method"><button role="tab" aria-selected={createMethod === 'ai'} className={createMethod === 'ai' ? 'active' : ''} onClick={() => setCreateMethod('ai')}><Sparkles size={17}/><span><b>AI generate</b><small>Turn intent into well-shaped Goals</small></span></button><button role="tab" aria-selected={createMethod === 'manual'} className={createMethod === 'manual' ? 'active' : ''} onClick={() => setCreateMethod('manual')}><Plus size={17}/><span><b>Manual</b><small>Define one broad outcome</small></span></button></div>{createMethod === 'manual' ? <form className="panel quest-form goal-form" onSubmit={createGoal}>{editorFields()}<div className="form-actions"><button type="button" className="secondary" onClick={() => setCreateOpen(false)}>Cancel</button><button className="primary" type="submit"><Plus size={17}/> Add goal</button></div></form> : <><section className="panel ai-goal-purpose"><label>What should this Goal do?<select value={aiGoalPurpose} onChange={(event) => setAiGoalPurpose(event.target.value as 'general' | 'skill')}><option value="general">Create a general Goal</option><option value="skill" disabled={!profileSkills.length}>Improve an existing skill</option></select></label>{aiGoalPurpose === 'skill' && <label>Skill to improve<select aria-label="Skill to improve" value={effectiveAiSkillId} onChange={(event) => setAiSkillId(event.target.value)}>{profileSkills.map((skill) => <option key={skill.id} value={skill.id}>{skill.name} · level {skill.level} → {skill.targetLevel}</option>)}</select><span className="field-guidance">The AI will use this skill's current level, target, experience, strengths, and growth areas.</span></label>}</section><AiImportPanel key={`goal-ai-${aiGoalPurpose}-${effectiveAiSkillId}`} target="quests" mode="create" focusSkillId={aiGoalPurpose === 'skill' ? effectiveAiSkillId : undefined}/></>}</section>}
