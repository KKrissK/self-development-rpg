...
describe('CV tuning prompt', () => {
  it('includes skill evidence and protects against invented CV claims', () => {
    const prompt = buildCvTuningPrompt({
      ...
      skills: [{ name: 'Playwright', category: 'Testing', level: 5, status: 'practicing', evidence: 'Built dashboard filtering tests.', assessment: { summary: 'Independent routine work.', strengths: ['API-first judgment'], gaps: ['Advanced flakiness diagnosis'] } }],
