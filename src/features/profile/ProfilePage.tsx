...
export function ProfilePage() {
  ...
  return <div className="page profile-page">
    ...
    <section className="panel data-panel"><div><h2>Appearance</h2><p>Switch between the Field Notes light theme and Night Console dark theme.</p></div><button className="secondary" onClick={() => update((current) => ({ ...current, theme: current.theme === 'dark' ? 'light' : 'dark' }))}>{state.theme === 'dark' ? <Sun size={18}/> : <Moon size={18}/>} {state.theme === 'dark' ? 'Light mode' : 'Dark mode'}</button></section>
