import { BookOpen, Bot, BriefcaseBusiness, CircleUserRound, Ellipsis, LayoutDashboard, ListChecks, ListTodo, Settings, Sparkles, TrendingUp, Trophy } from 'lucide-react'
import { useState, type ReactNode } from 'react'
import { brand } from '../config/brand'
import { useWorkspace } from '../app/AppState'
import { useI18n } from '../i18n/I18n'

export type PageId = 'dashboard' | 'skills' | 'achievements' | 'goals' | 'tasks' | 'library' | 'career' | 'coach' | 'profile' | 'settings'
...
export function Shell({ page, setPage, children }: { page: PageId; setPage: (page: PageId) => void; children: ReactNode }) {
  const { state, storageWarning } = useWorkspace()
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">{navigationGroups.map((group) => <section className="nav-group" key={group.label}><span className="nav-group-label">{group.label}</span>{group.ids.map((id) => { const item = items.find((candidate) => candidate.id === id)!; const Icon = item.icon; return <button key={id} aria-current={page === id ? 'page' : undefined} className={page === id ? 'nav-item active' : 'nav-item'} onClick={() => navigate(id)}><span className="nav-icon"><Icon size={18}/></span><span>{item.label}</span>{page === id && <i/>}</button> })}</section>)}<section className="nav-group nav-personal"><span className="nav-group-label">You</span><button aria-current={page === 'profile' ? 'page' : undefined} className={page === 'profile' ? 'nav-item active' : 'nav-item'} onClick={() => navigate('profile')}><span className="nav-icon"><CircleUserRound size={18}/></span><span>Profile</span>{page === 'profile' && <i/>}</button><button aria-current={page === 'settings' ? 'page' : undefined} className={page === 'settings' ? 'nav-item active' : 'nav-item'} onClick={() => navigate('settings')}><span className="nav-icon"><Settings size={18}/></span><span>Settings</span>{page === 'settings' && <i/>}</button></section></nav>
      <div className="sidebar-bottom">
        <div className="profile-settings-card"><button className="profile-chip" onClick={() => navigate('profile')}><span className="avatar">{profile.name.slice(0, 2).toUpperCase()}</span><span><b>{profile.name}</b><small>{profile.xp} XP across level {profile.level}</small></span><Sparkles size={14}/></button><button className="settings-page-button" aria-label="Open settings" aria-current={page === 'settings' ? 'page' : undefined} onClick={() => navigate('settings')}><Settings size={18}/></button></div>
      </div>
    </aside>
    <main className="main-content"><header className="workspace-bar"><div className="workspace-location"><span className="workspace-location-icon"><CurrentIcon size={16}/></span><b>{current.label}</b></div><span className="workspace-date">{today}</span><button className="workspace-settings-button" aria-label="Open settings" onClick={() => navigate('settings')}><Settings size={18}/></button><button className="workspace-avatar" onClick={() => navigate('profile')} aria-label={`Open ${profile.name} profile`}>{profile.name.slice(0, 2).toUpperCase()}</button></header>{storageWarning && <div className="warning" role="status">{storageWarning}</div>}<div className="page-stage" key={page}>{children}</div></main>
    {moreOpen && <div className="mobile-more" role="dialog" aria-label="More navigation"><button onClick={() => navigate('career')}><BriefcaseBusiness size={20}/> Career</button><button onClick={() => navigate('achievements')}><Trophy size={20}/> Achievements</button><button onClick={() => navigate('library')}><BookOpen size={20}/> Library</button><button onClick={() => navigate('coach')}><Bot size={20}/> AI Coach</button><button onClick={() => navigate('profile')}><CircleUserRound size={20}/> Profile & data</button><button onClick={() => navigate('settings')}><Settings size={20}/> Settings</button></div>}
    <nav className="mobile-nav" aria-label="Mobile navigation">{primaryMobile.map(({ id, label, icon: Icon }) => <button key={id} aria-current={page === id ? 'page' : undefined} className={page === id ? 'active' : ''} onClick={() => navigate(id)}><Icon size={20}/><span>{label}</span></button>)}<button aria-expanded={moreOpen} className={['career', 'achievements', 'library', 'coach', 'profile', 'settings'].includes(page) ? 'active' : ''} onClick={() => setMoreOpen((open) => !open)}><Ellipsis size={20}/><span>More</span></button></nav>
  </div>
}
