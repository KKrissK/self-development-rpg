...
const skill = z.object({ id: z.string(), profileId: z.string(), name: z.string().min(1).max(100), category: z.string().max(60), level: z.number().int().min(1).max(10), targetLevel: z.number().int().min(1).max(10), status: z.enum(['learning', 'practicing', 'proven']), evidence: z.string().max(1000) })
const quest = z.object({ id: z.string(), profileId: z.string(), title: z.string().min(1).max(160), notes: z.string().max(2000), priority: z.enum(['low', 'medium', 'high']), status: z.enum(['now', 'next', 'later', 'done']), xp: z.number().int().min(0).max(100), skillId: z.string().optional(), dueDate: z.string().optional(), completedAt: z.string().optional() })
...
const cv = z.object({ id: z.string(), profileId: z.string(), name: z.string(), employer: z.string(), role: z.string(), language: z.enum(['English', 'Hungarian', 'Other']), status: z.enum(['draft', 'ready', 'sent', 'archived']), fileName: z.string(), notes: z.string(), updatedAt: z.string() })
